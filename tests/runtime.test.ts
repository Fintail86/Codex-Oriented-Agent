...
describe("runtime setup", () => {
  it("creates Codex templates, an agent, and a session", async () => {
    ...
    const cosiaManifestJson = JSON.parse(await readFile(cosiaManifestPath, "utf8")) as { allowedTools: string[]; toolCatalogMigrationVersion: number };
    cosiaManifestJson.toolCatalogMigrationVersion = 1;
    cosiaManifestJson.allowedTools = ["read_file"];
    await writeFile(cosiaManifestPath, `${JSON.stringify(cosiaManifestJson, null, 2)}\n`, "utf8");
    const migratedCosiaAgent = await agents.loadAgent("cosia-agent");
    expect(migratedCosiaAgent.allowedTools).toContain("runtime_status_read");
    expect(migratedCosiaAgent.allowedTools).toContain("run_jobs_read");
    expect(migratedCosiaAgent.allowedTools).toContain("tool_growth_status_read");
    expect(migratedCosiaAgent.allowedTools).toContain("pending_actions_read");
    expect(migratedCosiaAgent.allowedTools).not.toContain("shell_request");
...
describe("policy core", () => {
  ...
  it("blocks stale Codex amendment apply and syncs POLICY.md when POLICY.json is amended", async () => {
    ...
    policy.version = "0.54.0-policy-amended";
    ...
    expect(await readFile(policyPath, "utf8")).toContain("0.54.0-policy-amended");
    expect(await readFile(join(root, "codex", "POLICY.md"), "utf8")).toContain("0.54.0-policy-amended");
  });
...
describe("status and listing", () => {
  ...
  it("exposes sanitized runtime inspector tools as read-only model tools", async () => {
    const root = await initializedWorkspace();
    const sessions = new SessionManager(root);
    const session = await sessions.createSession("cosia-agent", "Inspect runtime tools");
    const registry = new ToolRegistry();
    const allowedTools = ["runtime_status_read", "run_jobs_read", "tool_growth_status_read", "pending_actions_read"];
    const ledger = new RunJobLedger(root);
    const job = await ledger.create({
      ...
      request: "Check job with token=sk-runtimeinspectorsecret123456"
    });
    await ledger.update(job.id, {
      status: "succeeded",
      currentStep: "completed",
      finalOutputSummary: "Finished with api_key=sk-finalsecret123456789"
    });
    const shell = new ShellApprovalLedger(root).create({
      command: "node --version",
      reason: "Check runtime with token=sk-shellsecret123456789",
      sourceSessionId: session.id,
      sourceAgentId: "cosia-agent",
      sourceChannel: "cli"
    });
    ...
    const ctx = {
      ...
      gatewayRuntime: {
        ...
        providerId: "mock",
        currentToolGrowthRoutineId: undefined,
        pendingCommand: {
          id: "pending_1",
          commandId: "review.promote_memory",
          safety: "preview_mutation",
          createdAt: "2026-05-25T00:00:00.000Z",
          expiresAt: "2026-05-25T00:05:00.000Z",
          scope: { chatId: "123", sessionId: session.id }
        },
        pendingToolGrowthRequest: {
          request: "make runtime tool with token=sk-growthsecret123456789",
          capabilityName: "runtime_state_read",
          summary: "Inspect status without mutating anything.",
          readOnly: true,
          createdAt: "2026-05-25T00:00:00.000Z"
        }
      }
    ...
    const status = await registry.execute("runtime_status_read", {}, ctx);
    const jobs = await registry.execute("run_jobs_read", { includeTerminal: true }, ctx);
    const pending = await registry.execute("pending_actions_read", {}, ctx);

    expect(status.ok).toBe(true);
    expect(JSON.parse(status.content)).toMatchObject({ version: "0.54.0" });
    expect(jobs.ok).toBe(true);
    expect(jobs.content).toContain(job.id);
    expect(jobs.content).not.toContain("sk-runtimeinspectorsecret");
    expect(jobs.content).not.toContain("sk-finalsecret");
    expect(pending.ok).toBe(true);
    expect(pending.content).toContain(shell.id);
    expect(pending.content).toContain("pending_1");
    expect(pending.content).not.toContain("sk-shellsecret");
    expect(pending.content).not.toContain("sk-growthsecret");
  });

  it("scopes gateway runtime inspectors by role", async () => {
    const root = await initializedWorkspace();
    const sessions = new SessionManager(root);
    const session = await sessions.createSession("cosia-agent", "Inspect scoped jobs");
    const ledger = new RunJobLedger(root);
    await ledger.create({
      sessionId: session.id,
      providerId: "mock",
      source: { channel: "telegram", chatId: "123", chatType: "private", userId: "42" },
      request: "current chat job"
    });
    await ledger.create({
      sessionId: session.id,
      providerId: "mock",
      source: { channel: "telegram", chatId: "999", chatType: "private", userId: "99" },
      request: "other chat job"
    });
    const registry = new ToolRegistry();
    const baseCtx = {
      workspaceRoot: root,
      allowedTools: ["run_jobs_read"],
      sessionId: session.id,
      agentId: "cosia-agent",
      sourceChannel: "gateway" as const,
      gatewayActor: {
    ...
    const admin = await registry.execute("run_jobs_read", { scope: "all", includeTerminal: true }, {
      ...baseCtx,
      gatewayRole: "admin" as const
    });
    const master = await registry.execute("run_jobs_read", { scope: "all", includeTerminal: true }, {
      ...baseCtx,
      gatewayRole: "master" as const
    });
    const guest = await registry.execute("run_jobs_read", { includeTerminal: true }, {
      ...baseCtx,
      gatewayRole: "guest" as const
    });

    const adminJson = JSON.parse(admin.content) as { scope: string; notice: string; total: number };
    const masterJson = JSON.parse(master.content) as { scope: string; total: number };
    expect(adminJson.scope).toBe("current");
    expect(adminJson.notice).toContain("limited to current scope");
    expect(adminJson.total).toBe(1);
    expect(masterJson.scope).toBe("all");
    expect(masterJson.total).toBe(2);
    expect(guest.ok).toBe(false);
    expect(guest.content).toContain("role guest is below required role admin");
  });

  it("reads current tool growth routine status without exposing raw requests", async () => {
    const root = await initializedWorkspace();
    const sessions = new SessionManager(root);
    const session = await sessions.createSession("cosia-agent", "Inspect tool growth status");
    const result = await new ToolGrowthManager(root).start({
      request: "read-only status tool with token=sk-toolgrowthsecret123456789",
      agentId: "cosia-agent",
      providerId: "mock"
    });
    const output = await new ToolRegistry().execute("tool_growth_status_read", { scope: "current" }, {
      workspaceRoot: root,
      allowedTools: ["tool_growth_status_read"],
      sessionId: session.id,
      agentId: "cosia-agent",
    ...
    expect(output.content).toContain(result.routine.id);
    expect(output.content).toContain("candidate_ready");
    expect(output.content).not.toContain("sk-toolgrowthsecret");
  });
